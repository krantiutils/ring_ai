...
import { getNodeColor, NODE_ICON } from "@/features/flows/nodeRegistry";
...
export default function NodeCard({ data, selected }: NodeProps<FlowNode>) {
  ...
  // Expanded mode when configured and has preview text
  if (preview) {
    return (
      <div
  ...
  return (
    <div
      className={`${base} flex w-[140px] items-center gap-2 rounded-lg border-l-4 px-3 py-2.5`}
      style={{ borderLeftColor: color }}
    >
      <Handle type="target" position={Position.Left} className="!h-2.5 !w-2.5 !border-2 !border-[var(--border)] !bg-[var(--card)]" />
      {Icon && <Icon className="h-3.5 w-3.5 shrink-0" style={{ color }} />}
      <span className="text-xs font-medium text-[var(--foreground)] truncate">{data.label}</span>
      {isBranching ? (
        <>
          <Handle
            type="source"
