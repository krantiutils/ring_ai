...
export default function CreditPurchaseHistoryPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (search.trim()) {
        ...
        items = items.filter(
          (t) =>
            t.from_source.toLowerCase().includes(q) ||
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-2xl font-bold text-gray-900">Credit Purchase History</h1>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 text-left">
              ...
              <th className="px-5 py-3 font-semibold text-gray-600">From</th>
              <th className="px-5 py-3 font-semibold text-gray-600">Credit Type</th>
              <th className="px-5 py-3 font-semibold text-gray-600">Credit Rate</th>
              <th className="px-5 py-3 font-semibold text-gray-600">Credit Added</th>
            ...
            {loading ? (
              ...
              transactions.map((t, i) => (
                <tr key={t.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                  ...
                  <td className="px-5 py-3 text-gray-900">{t.from_source}</td>
                  <td className="px-5 py-3 text-gray-600">{t.credit_type}</td>
                  <td className="px-5 py-3 text-gray-600">NPR {t.credit_rate.toFixed(2)}</td>
                  <td className="px-5 py-3 font-medium text-green-600">+{formatNumber(t.amount)}</td>
