...
export default function CreditUsagePage() {
  ...
  const usages = (data?.items ?? []).filter((t) => t.type === "consume");

  const filtered = usages.filter((t) => {
                ...
                filtered.map((t, idx) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-white">
                      {t.description || t.reference_id || "Campaign"}
                    </td>
                    <td className="px-5 py-3 text-gray-300">Consume</td>
                    <td className="px-5 py-3 text-red-400">
                      -{Math.abs(t.amount).toFixed(1)}
                    </td>
